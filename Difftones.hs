...
instance Eq Pitch where
  p1 == p2 =
    ((abs (pc1n - pc2n)) < 0.1) && (oct1n == oct2n) where -- change 10/16/17: allow pcs to be "equal" if they're less than a cent apart. Change 10/28: extra set of parens around pc1-p2n because everything was turning out equal and it was a problem. Threshold changed to 10 cents.
    p1n = normalize p1
    pc1n = pc p1n
    oct1n = oct p1n
    p2n = normalize p2
...
-- an addition from 8/6/18, because I use it a lot
dft a b c d = difftone (Pitch a b) (Pitch c d)
